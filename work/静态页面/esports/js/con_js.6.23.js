...
var go_alert = function() {
	...
	$("body").append("<div class='alert_box' style='position:fixed;z-index:1500;width:100%;opacity:1;height:100%;left:0;top:0px;background-color:rgba(68,68,68,0.4);'><div class='content' style='width:260px;text-align:center;position:absolute;overflow: hidden;font-size:14px;left:50%;top:50%; transform:translateY(-50%) translateX(-50%);-webkit-transform:translateY(-50%) translateX(-50%);background-color:#fff;border-radius:5px'>"+(((json.tag_set&&json.tag_set.title_tag)||json.title_text)?"<h4 style='padding-top:15px;color:"+(json.title_text[1]||"#333")+"'>"+json.tag_set.title_tag+json.title_text[0]+"</h4>":"")+"<div style='padding:10px 20px; line-height:25px; border-bottom:1px solid #ccc;color:#999;font-size:14px'><div>"+(json.extra_dom.sort==1?json.extra_dom.html:"")+"</div><span style='display:inline-block;text-align:"+json.text_align+";color:"+(json.msg[1]||"#666")+"'>" + json.msg[0] + "</span><div>"+(json.extra_dom.sort==2?json.extra_dom.html:"")+"</div></div><div><button style='cursor:pointer;padding:10px 0px; background-color:#fff;border:0;display:block;width:100%;text-align:center;font-size:14px;outline:none;color:"+json.tag_set.agree_tag+(json.agree_text[1]||"initial")+"'>"+json.tag_set.agree_tag+json.agree_text[0]+"</button><div>");
